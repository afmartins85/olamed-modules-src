#ifndef BALANCEPROTOCOL_H
#define BALANCEPROTOCOL_H
...
class BalanceProtocol {
 public:
  ...
  void prepare_json_balance(double weigth, string m_address);
  bool prepare_json_balanceOff();

  inline string addr() const { return m_addr; }
  inline void setAddr(const string &addr) { m_addr = addr; }

  inline bool wasSentDisconnected() const { return m_wasSentDisconnected; }
  inline void setWasSentDisconnected(bool wasSentDisconnected) { m_wasSentDisconnected = wasSentDisconnected; }
