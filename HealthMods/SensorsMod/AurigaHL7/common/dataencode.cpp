...
bool parseInt64(int64_t &i, const std::string &s, const int base) {
  ...
  unsigned long long x =
      strtoull((negative) ? s.substr(1).c_str() : s.c_str(), &end, 0);
  // check for overflow
  if (errno == ERANGE) {
    return false;
  }
  ...
  // add sign and save returned data (does not change original data in case of
  // error)
...
bool parsePhoneNumber(int &country, int &sity, std::string &number,
                      int64_t &extension, int64_t &beeper, std::string &comment,
  ...
  std::transform(save_data.begin(), save_data.end(), save_data.begin(),
                 ::tolower);
  // find comment if any
  ...
  if (it != save_data.end()) {
    ...
    std::string::iterator it_e =
        std::find(save_data.begin(), save_data.end(), ')');
...
//  data - output data
//  [NN] [(999)]999-9999[X99999][B99999][C any text]
bool encodePhoneNumber(int country, int sity, const std::string &number,
                       int64_t extension, int64_t beeper,
                       const std::string &comment, std::string &data) {
  char cnv_data[6];
  int size = 9;
  data = "";
  if ((country < 0) || (country > 99) || (sity < 0) || (sity > 999) ||
      (number.length() != 8) || (number[3] != '-') || (extension < 0) ||
      (extension > 99999) || (beeper < 0) || (beeper > 99999))
    return false;
  if (country > 0) {
...
int parseTM(bool hourPrecision, const std::string &data, int &hour, int &minute,
            int &second, int &fractionOfSec, int &offset) {
  int params = -1;
  ...
  case 16: // HH[MM[SS[.S[S[S[S]]]]]][+/-ZZZZ]
    if (7 != sscanf(data.c_str(), "%2d%2d%2d.%4d%c%2d%2d%s", &hour, &minute,
                    &second, &fractionOfSec, &sign, &hOffset, &_offSet,
                    &_test_ch)) {
#ifdef TRACE_ENABLE
  ...
  case 15: // HH[MM[SS[.S[S[S]]]]][+/-ZZZZ]
    if (7 != sscanf(data.c_str(), "%2d%2d%2d.%3d%c%2d%2d%s", &hour, &minute,
                    &second, &fractionOfSec, &sign, &hOffset, &_offSet,
                    &_test_ch)) {
#ifdef TRACE_ENABLE
      std::cerr << "[verifyTM]: format not matched - <" << data << ">"
  ...
  case 14: // HH[MM[SS[.S[S]]]][+/-ZZZZ]
    if (7 != sscanf(data.c_str(), "%2d%2d%2d.%2d%c%2d%2d%s", &hour, &minute,
                    &second, &fractionOfSec, &sign, &hOffset, &_offSet,
                    &_test_ch)) {
#ifdef TRACE_ENABLE
      std::cerr << "[verifyTM]: format not matched - <" << data << ">"
  ...
  case 13: // HH[MM[SS[.S]]][+/-ZZZZ]
    if (7 != sscanf(data.c_str(), "%2d%2d%2d.%1d%c%2d%2d%s", &hour, &minute,
                    &second, &fractionOfSec, &sign, &hOffset, &_offSet,
                    &_test_ch)) {
#ifdef TRACE_ENABLE
      std::cerr << "[verifyTM]: format not matched - <" << data << ">"
  ...
  case 11:
    if (_omitOffsetFg) {
      // HH[MM[SS[.S[S[S[S]]]]]]
      if (4 != sscanf(data.c_str(), "%2d%2d%2d.%4d%s", &hour, &minute, &second,
                      &fractionOfSec, &_test_ch)) {
#ifdef TRACE_ENABLE
        std::cerr << "[verifyTM]: format not matched - <" << data << ">"
                  << std::endl;
#endif // TRACE_ENABLE
      ...
      // HH[MM[SS]][+/-ZZZZ]
      if (6 != sscanf(data.c_str(), "%2d%2d%2d%c%2d%2d%s", &hour, &minute,
                      &second, &sign, &hOffset, &_offSet, &_test_ch)) {
#ifdef TRACE_ENABLE
        std::cerr << "[verifyTM]: format not matched - <" << data << ">"
                  << std::endl;
#endif // TRACE_ENABLE
  ...
  case 10: // HH[MM[SS[.S[S[S]]]]]
    if (4 != sscanf(data.c_str(), "%2d%2d%2d.%3d%s", &hour, &minute, &second,
                    &fractionOfSec, &_test_ch)) {
#ifdef TRACE_ENABLE
      std::cerr << "[verifyTM]: format not matched - <" << data << ">"
                << std::endl;
  ...
  case 9:
    if (!_omitOffsetFg) {
      // HH[MM][+/-ZZZZ]
      if (5 != sscanf(data.c_str(), "%2d%2d%c%2d%2d%s", &hour, &minute, &sign,
                      &hOffset, &_offSet, &_test_ch)) {
#ifdef TRACE_ENABLE
        std::cerr << "[verifyTM]: format not matched - <" << data << ">"
                  << std::endl;
#endif // TRACE_ENABLE
      ...
      // HH[MM[SS[.S[S]]]]
      if (4 != sscanf(data.c_str(), "%2d%2d%2d.%2d%s", &hour, &minute, &second,
                      &fractionOfSec, &_test_ch)) {
#ifdef TRACE_ENABLE
        std::cerr << "[verifyTM]: format not matched - <" << data << ">"
                  << std::endl;
    ...
    break;
  case 8: // HH[MM[SS[.S]]]
    if (4 != sscanf(data.c_str(), "%2d%2d%2d.%1d%s", &hour, &minute, &second,
                    &fractionOfSec, &_test_ch)) {
#ifdef TRACE_ENABLE
      std::cerr << "[verifyTM]: format not matched - <" << data << ">"
                << std::endl;
  ...
  case 7:               // HH[+/-ZZZZ]
    ...
    if (4 != sscanf(data.c_str(), "%2d%c%2d%2d%s", &hour, &sign, &hOffset,
                    &_offSet, &_test_ch)) {
#ifdef TRACE_ENABLE
      std::cerr << "[verifyTM]: format not matched - <" << data << ">"
                << std::endl;
    ...
    break;
  case 6: // HH[MM[SS]]
    if (3 != sscanf(data.c_str(), "%2d%2d%2d%s", &hour, &minute, &second,
                    &_test_ch)) {
#ifdef TRACE_ENABLE
      std::cerr << "[verifyTM]: format not matched - <" << data << ">"
                << std::endl;
    ...
    break;
  case 4: // HH[MM]
    if (2 != sscanf(data.c_str(), "%2d%2d%s", &hour, &minute, &_test_ch)) {
#ifdef TRACE_ENABLE
      std::cerr << "[verifyTM]: format not matched - <" << data << ">"
                << std::endl;
#endif // TRACE_ENABLE
  ...
  if ((hour < MIN_HOUR) || (hour > MAX_HOUR) ||
      ((minute < MIN_MINUT) &&
       ((minute != EMPTY_MINUT) || (_omitOffsetFg && (params > 1)) ||
        (!_omitOffsetFg && (params > 2)))) ||
      (minute > MAX_MINUT) ||
      ((second < MIN_SECONDS) &&
       ((second != EMPTY_SECONDS) || (_omitOffsetFg && (params > 2)) ||
        (!_omitOffsetFg && (params > 3)))) ||
      (second > MAX_SECONDS) ||
      ((fractionOfSec < MIN_FRACTION_OF_SECONDS) &&
       ((fractionOfSec != EMPTY_FRACTION_OF_SECONDS) ||
        (_omitOffsetFg && (params > 3)) || (!_omitOffsetFg && (params > 4)))) ||
...
//  HH[MM[SS[.S[S[S[S]]]]]][+/-ZZZZ] / HHMM[SS[.S[S[S[S]]]]][+/-ZZZZ]
bool encodeTM(std::string &data, int hour, int minute, int second,
              int fractionOfSec, char precision, int offset) {
  char formatter[6];
  std::string saver;
  // check precision
  if ((precision != 'H') && (precision != 'M') && (precision != 'S') &&
      (precision != 'h') && (precision != 'm') && (precision != 's') &&
      (precision != 0))
    return false;
  // check data borders
  if ((hour < MIN_HOUR) || (hour > MAX_HOUR))
...
//  day - output day
//  YYYY[MM[DD]]
int parseDT(const std::string &data, int &year, int &month, int &day) {
  int params = -1;
  char _test_ch[10];
  year = INIT_YEAR;
  month = EMPTY_MONTH;
  ...
  case 8: // YYYY[MM[DD]]
    if (3 !=
        sscanf(data.c_str(), "%4d%2d%2d%s", &year, &month, &day, &_test_ch)) {
#ifdef TRACE_ENABLE
    ...
    break;
  case 6: // YYYY[MM]
    if (2 != sscanf(data.c_str(), "%4d%2d%s", &year, &month, &_test_ch)) {
#ifdef TRACE_ENABLE
      std::cerr << "[verifyDT]: format not matched - <" << data << ">"
                << std::endl;
#endif // TRACE_ENABLE
  ...
  if ((year < MIN_YEAR) || (month > MAX_MONTH) ||
      ((month < MIN_MONTH) && ((month != EMPTY_MONTH) || (params > 1))) ||
      (day > MAX_DAY) ||
      ((day < MIN_DAY) && ((day != EMPTY_DAY) || (params > 2)))) {
#ifdef TRACE_ENABLE
...
//  day - input day
//  YYYY[MM[DD]]
bool encodeDT(std::string &data, int year, int month, int day) {
  char formatter[5];
  std::string saver;
  // check data borders
  if (year < MIN_YEAR)
...
int parseTS(const std::string &data, int &year, int &month, int &day, int &hour,
            int &minute, int &second, int &fractionOfSec, int &offset) {
  int paramsDT = -1;
  ...
  if (data.length() > MAX_DATE_SIZE) {
    int paramsTM = -1;
    paramsTM = parseTM(true, data.substr(MAX_DATE_SIZE), hour, minute, second,
...
//  offset - input time offset in format +/-M as number or default = 9999
//  YYYY[MM[DD[HH[MM[SS[.S[S[S[S]]]]]]]]][+/-ZZZZ]
bool encodeTS(std::string &data, int year, int month, int day, int hour,
              int minute, int second, int fractionOfSec, int offset) {
  std::string saver;
