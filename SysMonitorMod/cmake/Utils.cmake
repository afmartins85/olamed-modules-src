...
#
option(PRJ_DUMP_HOOKS_FLOW "Dump called PRJ hooks" OFF)
macro(prj_cmake_hook_append hook_name)
  set(__var_name "__PRJ_CMAKE_HOOKS_${hook_name}")
...
endmacro()
macro(prj_cmake_hook hook_name)
  set(__var_name "__PRJ_CMAKE_HOOKS_${hook_name}")
  if(PRJ_DUMP_HOOKS_FLOW)
  ...
  endforeach()
endmacro()
macro(prj_cmake_reset_hooks)
  get_cmake_property(__variableNames VARIABLES)
  foreach(__variableName ${__variableNames})
    if(__variableName MATCHES "^__PRJ_CMAKE_HOOKS_")
      unset(${__variableName})
      unset(${__variableName} CACHE)
    endif()
  endforeach()
endmacro()
macro(prj_cmake_hook_register_dir dir)
  file(GLOB hook_files RELATIVE "${dir}" "${dir}/*.cmake")
  foreach(f ${hook_files})
    if(f MATCHES "^(.+)\\.cmake$")
      set(hook_name "${CMAKE_MATCH_1}")
      prj_cmake_hook_append(${hook_name} "${dir}/${f}")
    endif()
  endforeach()
endmacro()


function(prj_cmake_eval var_name)
  if(DEFINED ${var_name})
...
macro(prj_update VAR)
  if(NOT DEFINED ${VAR})
    ...
    #prj_debug_message("Preserve old value for ${VAR}: ${${VAR}}")
  endif()
endmacro()
...
# Usage samples:
#   prj_assert(MyLib_FOUND)
#   prj_assert(DEFINED MyLib_INCLUDE_DIRS)
macro(prj_assert)
  if(NOT (${ARGN}))
...
macro(prj_debug_message)
#  string(REPLACE ";" " " __msg "${ARGN}")
#  message(STATUS "${__msg}")
endmacro()

macro(prj_check_environment_variables)
  foreach(_var ${ARGN})
...
macro(prj_path_join result_var P1 P2_)
...
macro(_prj_fix_target target_var)
  if(BUILD_project_world)
    if(PRJ_MODULE_${${target_var}}_IS_PART_OF_WORLD)
      set(${target_var} project_world)
    endif()
  endif()
endmacro()
...
function(prj_is_subdir res dir sub )
...
function(prj_is_project_directory result_var dir)
  set(result FALSE)
  foreach(parent ${PRJ_SOURCE_DIR} ${PRJ_BINARY_DIR} ${PRJ_EXTRA_MODULES_PATH})
    prj_is_subdir(result "${parent}" "${dir}")
    if(result)
      break()
    endif()
  endforeach()
...
# adds include directories in such a way that directories from the PRJ source tree go first
function(prj_include_directories)
  prj_debug_message("prj_include_directories( ${ARGN} )")
  set(__add_before "")
  foreach(dir ${ARGN})
    prj_is_project_directory(__is_project_dir "${dir}")
    if(__is_project_dir)
...
function(prj_append_target_property target prop)
...
function(prj_append_dependant_targets target)
  #prj_debug_message("prj_append_dependant_targets(${target} ${ARGN})")
  _prj_fix_target(target)
  set(PRJ_DEPENDANT_TARGETS_${target} "${PRJ_DEPENDANT_TARGETS_${target}};${ARGN}" CACHE INTERNAL "" FORCE)
endfunction()

# adds include directories in such a way that directories from the PRJ source tree go first
function(prj_target_include_directories target)
  #prj_debug_message("prj_target_include_directories(${target} ${ARGN})")
  _prj_fix_target(target)
  set(__params "")
  ...
  if(HAVE_CUDA OR CMAKE_VERSION VERSION_LESS 2.8.11)
	...
	if(PRJ_DEPENDANT_TARGETS_${target})
	  foreach(t ${PRJ_DEPENDANT_TARGETS_${target}})
	...
	if(PRJ_DEPENDANT_TARGETS_${target})
      ...
      if(__params)
	set(__new_inc ${prj_TARGET_INCLUDE_DIRS_${target}})
        list(APPEND __new_inc ${__params})
	set(prj_TARGET_INCLUDE_DIRS_${target} "${__new_inc}" CACHE INTERNAL "")
      endif()
      if(__system_params)
	set(__new_inc ${prj_TARGET_INCLUDE_SYSTEM_DIRS_${target}})
        list(APPEND __new_inc ${__system_params})
	set(prj_TARGET_INCLUDE_SYSTEM_DIRS_${target} "${__new_inc}" CACHE INTERNAL "")
      endif()
    endif()
  endif()
endfunction()
...
MACRO(prj_check_compiler_flag LANG FLAG RESULT)
  set(_fname "${ARGN}")
  if(NOT DEFINED ${RESULT})
	...
	foreach(_regex ${prj_COMPILER_FAIL_REGEX})
          if(NOT ${RESULT})
            break()
          endif()
...
macro(prj_check_flag_support lang flag varname base_options)
  if(CMAKE_BUILD_TYPE)
  ...
  prj_check_compiler_flag("${_lang}" "${base_options} ${flag}" ${${varname}} ${ARGN})
endmacro()

# turns off warnings
macro(prj_warnings_disable)
  if(NOT ENABLE_NOISY_WARNINGS)
    ...
    elseif((CMAKE_COMPILER_IS_GNUCXX OR (UNIX AND CV_ICC)) AND _gxx_warnings AND _flag_vars)
	  ...
	  prj_check_flag_support(${var} "${warning}" _varname "")
          if(${_varname})
            set(${var} "${${var}} ${warning}")
          endif()
        endforeach()
...
macro(prj_append_source_file_compile_definitions source)
...
macro(add_apple_compiler_options the_module)
  prj_check_flag_support(OBJCXX "-fobjc-exceptions" HAVE_OBJC_EXCEPTIONS "")
  if(HAVE_OBJC_EXCEPTIONS)
    foreach(source ${PRJ_MODULE_${the_module}_SOURCES})
...
macro(prj_OPTION variable description value)
  set(__value ${value})
  set(__condition "")
...
# Usage: prj_append_build_options(HIGHGUI FFMPEG)
macro(prj_append_build_options var_prefix pkg_prefix)
...
macro(prj_check_modules define)
  unset(HAVE_${define})
  foreach(m ${ARGN})
    if (m MATCHES "(.*[^><])(>=|=|<=)(.*)")
...
macro(CHECK_MODULE module_name define cv_module)
  ...
  if(PKG_CONFIG_FOUND)
    ...
    if(${ALIAS_FOUND})
      set(${define} 1)
      prj_append_build_options(${cv_module} ${ALIAS})
    endif()
  endif()
endmacro()
...
prj_OPTION(BUILD_USE_SYMLINKS "Use symlinks instead of files copying during build (and !!INSTALL!!)" (${__symlink_default}) IF (UNIX OR DEFINED __symlink_default))

if(CMAKE_VERSION VERSION_LESS "3.2")
  macro(prj_cmake_byproducts var_name)
...
set(PRJ_DEPHELPER "${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/dephelper" CACHE INTERNAL "")
file(MAKE_DIRECTORY ${PRJ_DEPHELPER})
...
set(PRJ_BUILD_INFO_STR "" CACHE INTERNAL "")
function(prj_output_status msg)
  message(STATUS "${msg}")
  string(REPLACE "\\" "\\\\" msg "${msg}")
  string(REPLACE "\"" "\\\"" msg "${msg}")
  set(PRJ_BUILD_INFO_STR "${PRJ_BUILD_INFO_STR}\"${msg}\\n\"\n" CACHE INTERNAL "")
endfunction()

macro(prj_finalize_status)
  set(PRJ_BUILD_INFO_FILE "${CMAKE_BINARY_DIR}/version_string.tmp")
  if(EXISTS "${PRJ_BUILD_INFO_FILE}")
    file(READ "${PRJ_BUILD_INFO_FILE}" __content)
  else()
    set(__content "")
  endif()
  if("${__content}" STREQUAL "${PRJ_BUILD_INFO_STR}")
    #message(STATUS "${PRJ_BUILD_INFO_FILE} contains the same content")
  else()
    file(WRITE "${PRJ_BUILD_INFO_FILE}" "${PRJ_BUILD_INFO_STR}")
  endif()
  unset(__content)
  unset(PRJ_BUILD_INFO_STR CACHE)

  if(NOT PRJ_SKIP_STATUS_FINALIZATION)
    if(DEFINED PRJ_MODULE_project_core_BINARY_DIR)
      execute_process(COMMAND ${CMAKE_COMMAND} -E copy_if_different "${PRJ_BUILD_INFO_FILE}" "${PRJ_MODULE_project_core_BINARY_DIR}/version_string.inc" OUTPUT_QUIET)
    endif()
  endif()

  if(UNIX)
    install(FILES "${PRJ_SOURCE_DIR}/platforms/scripts/valgrind.supp"
	          "${PRJ_SOURCE_DIR}/platforms/scripts/valgrind_3rdparty.supp"
		DESTINATION "${PRJ_OTHER_INSTALL_PATH}" COMPONENT "dev")
  endif()
endmacro()
...
function(status text)
  ...
  if(DEFINED status_cond)
	...
	prj_output_status("${status_text} ${status_then}")
      else()
        string(REPLACE ";" " " status_else "${status_else}")
        string(REGEX REPLACE "^[ \t]+" "" status_else "${status_else}")
	prj_output_status("${status_text} ${status_else}")
      endif()
    else()
      string(REPLACE ";" " " status_cond "${status_cond}")
      string(REGEX REPLACE "^[ \t]+" "" status_cond "${status_cond}")
      prj_output_status("${status_text} ${status_cond}")
    endif()
  else()
    prj_output_status("${text}")
  endif()
...
#
# Usage:
#   prj_build_features_string(out [EXCLUSIVE] [IF feature THEN title] ... [ELSE title])
#
function(prj_build_features_string out)
  set(result)
...
macro(prj_list_filterout lst regex)
  foreach(item ${${lst}})
...
macro(prj_list_filter lst regex)
  set(dst ${ARGN})
  if(NOT dst)
    set(dst ${lst})
  endif()
...
macro(prj_list_add_prefix LST PREFIX)
  set(__tmp "")
  foreach(item ${${LST}})
...
macro(prj_list_add_suffix LST SUFFIX)
  set(__tmp "")
  foreach(item ${${LST}})
...
macro(prj_list_pop_front LST VAR)
  if(${LST})
    list(GET ${LST} 0 ${VAR})
...
# prj_get_duplicates(<output list> <element> [<element> ...])
function(prj_get_duplicates res)
  if(ARGC LESS 2)
    message(FATAL_ERROR "Invalid call to prj_get_duplicates")
  endif()
  set(lst ${ARGN})
  list(SORT lst)
  set(prev_item)
...
macro(prj_regex_escape var regex)
  string(REGEX REPLACE "([+.*^$])" "\\\\1" ${var} "${regex}")
endmacro()


# convert list of paths to full paths
macro(prj_convert_to_full_paths VAR)
  if(${VAR})
    set(__tmp "")
    foreach(path ${${VAR}})
...
function(prj_convert_to_lib_name var)
  set(tmp "")
  foreach(path ${ARGN})
    get_filename_component(tmp_name "${path}" NAME)
    prj_get_libname(tmp_name "${tmp_name}")
...
# add install command
function(prj_install_target)
  if(APPLE_FRAMEWORK AND BUILD_SHARED_LIBS)
    install(TARGETS ${ARGN} FRAMEWORK DESTINATION ${PRJ_3P_LIB_INSTALL_PATH})
  else()
...
macro(prj_parse_header FILENAME FILE_VAR)
  set(vars_regex "")
...
macro(prj_parse_header2 LIBNAME HDR_PATH VARNAME)
  prj_clear_vars(${LIBNAME}_VERSION_MAJOR
...
macro(prj_parse_pkg LIBNAME PKG_PATH SCOPE)
...
function(prj_source_group group)
  if(BUILD_project_world AND PRJ_MODULE_${the_module}_IS_PART_OF_WORLD)
...
macro(__prj_push_target_link_libraries)
  if(NOT TARGET ${target})
    if(NOT DEFINED PRJ_MODULE_${target}_LOCATION)
      message(FATAL_ERROR "prj_target_link_libraries: invalid target: '${target}'")
    endif()
    set(PRJ_MODULE_${target}_LINK_DEPS ${PRJ_MODULE_${target}_LINK_DEPS} ${ARGN} CACHE INTERNAL "" FORCE)
  else()
    target_link_libraries(${target} ${ARGN})
  endif()
endmacro()

function(prj_target_link_libraries target)
  set(LINK_DEPS ${ARGN})
  _prj_fix_target(target)
  ...
  foreach(dep ${LINK_DEPS})
	...
	__prj_push_target_link_libraries(${LINK_MODE} ${LINK_PENDING})
        set(LINK_PENDING "")
        set(LINK_MODE "${dep}")
      endif()
    else()
      if(BUILD_project_world)
	if(PRJ_MODULE_${dep}_IS_PART_OF_WORLD)
	  set(dep project_world)
        endif()
      endif()
...
function(prj_target_compile_definitions target)
  _prj_fix_target(target)
  if(NOT TARGET ${target})
    if(NOT DEFINED PRJ_MODULE_${target}_LOCATION)
      message(FATAL_ERROR "prj_target_compile_definitions: invalid target: '${target}'")
    endif()
    set(PRJ_MODULE_${target}_COMPILE_DEFINITIONS ${PRJ_MODULE_${target}_COMPILE_DEFINITIONS} ${ARGN} CACHE INTERNAL "" FORCE)
  else()
...
function(_prj_append_target_includes target)
  if(DEFINED prj_TARGET_INCLUDE_DIRS_${target})
    target_include_directories(${target} PRIVATE ${prj_TARGET_INCLUDE_DIRS_${target}})
    if(PRJ_DEPENDANT_TARGETS_${target})
      foreach(t ${PRJ_DEPENDANT_TARGETS_${target}})
	target_include_directories(${t} PRIVATE ${prj_TARGET_INCLUDE_DIRS_${target}})
      endforeach()
    endif()
    unset(prj_TARGET_INCLUDE_DIRS_${target} CACHE)
  endif()

  if(DEFINED prj_TARGET_INCLUDE_SYSTEM_DIRS_${target})
    target_include_directories(${target} SYSTEM PRIVATE ${prj_TARGET_INCLUDE_SYSTEM_DIRS_${target}})
    if(PRJ_DEPENDANT_TARGETS_${target})
      foreach(t ${PRJ_DEPENDANT_TARGETS_${target}})
	target_include_directories(${t} SYSTEM PRIVATE ${prj_TARGET_INCLUDE_SYSTEM_DIRS_${target}})
      endforeach()
    endif()
    unset(prj_TARGET_INCLUDE_SYSTEM_DIRS_${target} CACHE)
  endif()
endfunction()

function(prj_add_executable target)
  add_executable(${target} ${ARGN})
  _prj_append_target_includes(${target})
endfunction()

function(prj_add_library target)
  if(HAVE_CUDA AND ARGN MATCHES "\\.cu")
    prj_include_directories(${CUDA_INCLUDE_DIRS})
    prj_cuda_compile(cuda_objs ${ARGN})
    set(PRJ_MODULE_${target}_CUDA_OBJECTS ${cuda_objs} CACHE INTERNAL "Compiled CUDA object files")
  endif()
  ...
  if(APPLE_FRAMEWORK AND BUILD_SHARED_LIBS)
    ...
    set_target_properties(${target} PROPERTIES
      FRAMEWORK TRUE
      MACOSX_FRAMEWORK_IDENTIFIER org.project
      MACOSX_FRAMEWORK_INFO_PLIST ${CMAKE_BINARY_DIR}/ios/Info.plist
      # "current version" in semantic format in Mach-O binary file
      VERSION ${PRJ_LIBVERSION}
      # "compatibility version" in semantic format in Mach-O binary file
      SOVERSION ${PRJ_LIBVERSION}
      INSTALL_RPATH ""
      INSTALL_NAME_DIR "@rpath"
      BUILD_WITH_INSTALL_RPATH 1
      LIBRARY_OUTPUT_NAME "project2"
      XCODE_ATTRIBUTE_TARGETED_DEVICE_FAMILY "1,2"
      #PUBLIC_HEADER "${PRJ_CONFIG_FILE_INCLUDE_DIR}/cvconfig.h"
...
macro(prj_get_libname var_name)
  get_filename_component(__libname "${ARGN}" NAME)
  # libproject_core.so.3.3 -> project_core
  string(REGEX REPLACE "^lib(.+)\\.(a|so|dll)(\\.[.0-9]+)?$" "\\1" __libname "${__libname}")
  # MacOSX: libproject_core.3.3.1.dylib -> project_core
  string(REGEX REPLACE "^lib(.+[^.0-9])\\.([.0-9]+\\.)?dylib$" "\\1" __libname "${__libname}")
  set(${var_name} "${__libname}")
endmacro()

# build the list of project libs and dependencies for all modules
...
macro(prj_get_all_libs _modules _extra _3rdparty)
  set(${_modules} "")
  set(${_extra} "")
  set(${_3rdparty} "")
  foreach(m ${PRJ_MODULES_PUBLIC})
    if(TARGET ${m})
    ...
    prj_list_reverse(_rev_deps)
    foreach (dep ${_rev_deps})
      if(DEFINED PRJ_MODULE_${dep}_LOCATION)
        list(INSERT ${_modules} 0 ${dep})
      endif()
    endforeach()
    foreach (dep ${deps} ${PRJ_LINKER_LIBS})
      if (NOT DEFINED PRJ_MODULE_${dep}_LOCATION)
        ...
        if (TARGET ${dep})
          ...
          if(_type STREQUAL "STATIC_LIBRARY" AND BUILD_SHARED_LIBS OR _type STREQUAL "INTERFACE_LIBRARY")
            ...
            if (_POS EQUAL 0)
		prj_get_libname(_libname "${_output_name}")
  ...
  prj_list_filterout(${_modules} "^[\$]<")
  prj_list_filterout(${_3rdparty} "^[\$]<")
  prj_list_filterout(${_extra} "^[\$]<")

  # convert CMake lists to makefile literals
  foreach(lst ${_modules} ${_3rdparty} ${_extra})
    prj_list_unique(${lst})
    prj_list_reverse(${lst})
  endforeach()
endmacro()


function(prj_add_test_from_target test_name test_kind the_target)
  if(CMAKE_VERSION VERSION_GREATER "2.8" AND NOT CMAKE_CROSSCOMPILING)
    ...
    set_tests_properties("${test_name}" PROPERTIES
      LABELS "${PRJ_MODULE_${the_module}_LABEL};${test_kind}"
      WORKING_DIRECTORY "${test_report_dir}")

    if(PRJ_TEST_DATA_PATH)
      set_tests_properties("${test_name}" PROPERTIES
	ENVIRONMENT "PRJ_TEST_DATA_PATH=${PRJ_TEST_DATA_PATH}")
    endif()
  endif()
endfunction()

macro(prj_add_testdata basedir dest_subdir)
  if(BUILD_TESTS)
    if(NOT CMAKE_CROSSCOMPILING AND NOT INSTALL_TESTS)
      file(COPY ${basedir}/
	   DESTINATION ${CMAKE_BINARY_DIR}/${PRJ_TEST_DATA_INSTALL_PATH}/${dest_subdir}
           ${ARGN}
      )
    endif()
    if(INSTALL_TESTS)
      install(DIRECTORY ${basedir}/
	      DESTINATION ${PRJ_TEST_DATA_INSTALL_PATH}/${dest_subdir}
              COMPONENT "tests"
              ${ARGN}
      )
    endif()
...
macro(prj_generate_vs_version_file DESTINATION)
  ...
  macro(__vs_ver_update_variable name)
    if(VS_VER_NAME AND DEFINED PRJ_${VS_VER_NAME}_VS_VER_${name})
      set(PRJ_VS_VER_${name} "${PRJ_${VS_VER_NAME}_VS_VER_${name}}")
    elseif(VS_VER_${name})
      set(PRJ_VS_VER_${name} "${VS_VER_${name}}")
    endif()
  endmacro()
  ...
  macro(__vs_ver_update_str_variable name)
    if(VS_VER_NAME AND DEFINED PRJ_${VS_VER_NAME}_VS_VER_${name})
      set(PRJ_VS_VER_${name}_STR "${PRJ_${VS_VER_NAME}_VS_VER_${name}}")
    elseif(VS_VER_${name})
      set(PRJ_VS_VER_${name}_STR "${VS_VER_${name}}")
    endif()
  endmacro()
  ...
  if(PRJ_VS_VER_COPYRIGHT_STR)
    set(PRJ_VS_VER_HAVE_COPYRIGHT_STR 1)
  else()
    set(PRJ_VS_VER_HAVE_COPYRIGHT_STR 0)
  endif()

  if(PRJ_VS_VER_COMMENTS_STR)
    set(PRJ_VS_VER_HAVE_COMMENTS_STR 1)
  else()
    set(PRJ_VS_VER_HAVE_COMMENTS_STR 0)
  endif()

  configure_file("${PRJ_SOURCE_DIR}/cmake/templates/vs_version.rc.in" "${DESTINATION}" @ONLY)
endmacro()

macro(prj_cmake_script_append_var content_var)
...
macro(prj_copyfiles_append_file list_var src dst)
...
macro(prj_copyfiles_append_dir list_var src dst)
  set(__glob ${ARGN})
  list(LENGTH ${list_var} __id)
  list(APPEND ${list_var} ${__id})
  set(${list_var}_SRC_${__id} "${src}")
...
macro(prj_copyfiles_make_config_string content_var list_var)
...
macro(prj_copyfiles_make_config_file filename_var list_var)
  prj_copyfiles_make_config_string(${list_var}_CONFIG ${list_var})
  set(${filename_var} "${CMAKE_CURRENT_BINARY_DIR}/copyfiles-${list_var}.cmake")
  file(WRITE "${${filename_var}}" "${${list_var}_CONFIG}")
endmacro()

macro(prj_copyfiles_add_forced_target target list_var comment_str)
  prj_copyfiles_make_config_file(CONFIG_FILE ${list_var})
  prj_cmake_byproducts(__byproducts BYPRODUCTS "${PRJ_DEPHELPER}/${target}")
  add_custom_target(${target}
      ${__byproducts}  # required for add_custom_target() by ninja
      COMMAND ${CMAKE_COMMAND}
        "-DCONFIG_FILE:PATH=${CONFIG_FILE}"
        "-DCOPYLIST_VAR:STRING=${list_var}"
	"-DDEPHELPER=${PRJ_DEPHELPER}/${target}"
	-P "${PRJ_SOURCE_DIR}/cmake/copy_files.cmake"
      WORKING_DIRECTORY ${CMAKE_BINARY_DIR}
      COMMENT "${comment_str}"
      DEPENDS "${PRJ_SOURCE_DIR}/cmake/copy_files.cmake"
              # ninja warn about file(WRITE): "${SRC_COPY_CONFIG_FILE}"
  )
endmacro()

macro(prj_copyfiles_add_target target list_var comment_str)
  set(deps ${ARGN})
  prj_copyfiles_make_config_file(CONFIG_FILE ${list_var})
  add_custom_command(OUTPUT "${PRJ_DEPHELPER}/${target}"
      COMMAND ${CMAKE_COMMAND}
        "-DCONFIG_FILE:PATH=${CONFIG_FILE}"
        "-DCOPYLIST_VAR:STRING=${list_var}"
	"-DDEPHELPER=${PRJ_DEPHELPER}/${target}"
	-P "${PRJ_SOURCE_DIR}/cmake/copy_files.cmake"
      WORKING_DIRECTORY ${CMAKE_BINARY_DIR}
      COMMENT "${comment_str}"
      DEPENDS "${PRJ_SOURCE_DIR}/cmake/copy_files.cmake" ${deps}
              # ninja warn about file(WRITE): "${SRC_COPY_CONFIG_FILE}"
  )
  add_custom_target(${target} DEPENDS "${PRJ_DEPHELPER}/${target}")
endmacro()

macro(prj_get_smart_file_name output_var fpath)
  prj_is_subdir(__subir "${PRJ_BINARY_DIR}" "${fpath}")
  if(__subir)
    file(RELATIVE_PATH ${output_var} "${PRJ_BINARY_DIR}" "${fpath}")
    set(${output_var} "<BUILD>/${${output_var}}")
  else()
    prj_is_subdir(__subir "${PRJ_SOURCE_DIR}" "${fpath}")
    if(__subir)
      file(RELATIVE_PATH ${output_var} "${PRJ_SOURCE_DIR}" "${fpath}")
    else()
